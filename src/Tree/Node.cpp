#include "Node.h"

Node::Node() : Node(Point(-1000, -1000, -1000), Point(1000, 1000, 1000)) {}

Node::Node(Point start, Point end) : Node(start, end, {}, nullptr, nullptr, nullptr) {}

Node::Node(Point start, Point end, vector<Triangle> list) : Node(start, end, list, nullptr, nullptr, nullptr) {}

Node::Node(Point start, Point end, Node *p) : Node(start, end, {}, nullptr, nullptr, p) {}

Node::Node(Node *l, Node *r, Node *p) : Node(p->start(), p->end(), {}, l, r, p) {}

Node::Node(Point start_p, Point end_p, vector<Triangle> list, Node *l, Node *r, Node *p) {
	setStart(start_p);
	setEnd(end_p);
	setTriangles(list);
	setLeft(l);
	setRight(r);
	setParent(p);
}

Node *Node::createNode(vector<Triangle> list) {
	Node *tree = new Node();
	tree->fitBox(tree, list);
	tree->setTriangles(list);
	tree->fitAllBoxes();
	return tree;
}

void Node::setStart(Point start) { this->startP = start; }

void Node::setEnd(Point end) { this->endP = end; }

void Node::setLeft(Node *l) { this->leftN = l; }

void Node::setRight(Node *r) { this->rightN = r; }

void Node::setParent(Node *p) { this->parentN = p; }

void Node::setTriangles(vector<Triangle> t) {
	this->trianglesList = t;
	//fitBox();
	if (trianglesCount() >= MAX_SIZE) divide();
}

void Node::addTriangles(vector<Triangle> t) {
	this->trianglesList.insert(this->trianglesList.end(), t.begin(), t.end());
	//fitBox();
	if (trianglesCount() >= MAX_SIZE) divide();
}

void Node::addTriangle(Triangle t) {

	if (left() == nullptr && right() == nullptr) {
		this->trianglesList.push_back(t);
		if (MAX_SIZE + 1 >= trianglesCount() >= MAX_SIZE - 1) {
			fitBox();
		}
	} else {
		this->setTriangleToSide(t);
	}
	if (trianglesCount() >= MAX_SIZE) divide();
}

bool Node::isPointInBox(Point p, Point s, Point e) {
	bool ans1 = std::min(s.x(), e.x()) - EPSILON <= p.x() <= std::max(s.x(), e.x()) + EPSILON;
	bool ans2 = std::min(s.y(), e.y()) - EPSILON <= p.y() <= std::max(s.y(), e.y()) + EPSILON;
	bool ans3 = std::min(s.z(), e.z()) - EPSILON <= p.z() <= std::max(s.z(), e.z()) + EPSILON;
	return ans1 && ans2 && ans3;
}

void Node::setTriangleToSide(Triangle t) {
	bool ansLeft = true;
	Point m = (start() + end()) * 0.5;
	Point c = t.center();
	auto tmp_l = Point(end());
	auto tmp_r = Point(start());
	//findDivIndex();
	switch (divIndex) {
		case 0: tmp_l.setX(m.x());
			tmp_r.setX(m.x());
			break;
		case 1: tmp_l.setY(m.y());
			tmp_r.setY(m.y());
			break;
		case 2: tmp_l.setZ(m.z());
			tmp_r.setZ(m.z());
			break;
	}
	ansLeft = isPointInBox(c, start(), tmp_l);
	if (ansLeft) {
		if (left() != nullptr) { left()->addTriangle(t); }
		else {
			Node *l = new Node(start(), tmp_l, this);
			l->deep = deep + 1;
			setLeft(l);
			l->addTriangle(t);
		}
	} else {
		if (right() != nullptr) {
			right()->addTriangle(t);
		} else {
			Node *r = new Node(tmp_r, end(), this);
			r->deep = deep + 1;
			setRight(r);
			r->addTriangle(t);
		}
	}
}

Node *Node::right() { return this->rightN; }

Node *Node::left() { return this->leftN; }

Node *Node::parent() { return this->parentN; }

Point Node::start() { return this->startP; }

Point Node::end() { return this->endP; }

vector<Triangle> Node::triangles() { return this->trianglesList; }

int Node::trianglesCount() { return this->trianglesList.size(); }

void Node::fitBox() {
	Point max_t = Point(-1000, -1000, -1000);
	Point min_t = Point(1000, 1000, 1000);
	Point tmp = Point();
	for (auto &t : this->trianglesList) {
		tmp = t.center();
		vector<Point> allPoints = {t.v0(), t.v1(), t.v2()};
		for (auto &p : allPoints) {
			if (p.x() > max_t.x()) max_t.setX(p.x());
			if (p.y() > max_t.y()) max_t.setY(p.y());
			if (p.z() > max_t.z()) max_t.setZ(p.z());
			if (p.x() < min_t.x()) min_t.setX(p.x());
			if (p.y() < min_t.y()) min_t.setY(p.y());
			if (p.z() < min_t.z()) min_t.setZ(p.z());
		}
	}
	endP.setCoordinates(max_t.x(), max_t.y(), max_t.z());
	startP.setCoordinates(min_t.x(), min_t.y(), min_t.z());
}

void Node::fitBox(Node *n, vector<Triangle> v) {
	Point max_t = Point(-1000, -1000, -1000);
	Point min_t = Point(1000, 1000, 1000);
	Point tmp = Point();
	for (auto &t : v) {
		vector<Point> allPoints = {t.v0(), t.v1(), t.v2()};
		for (auto &p : allPoints) {
			if (p.x() > max_t.x()) max_t.setX(p.x());
			if (p.y() > max_t.y()) max_t.setY(p.y());
			if (p.z() > max_t.z()) max_t.setZ(p.z());
			if (p.x() < min_t.x()) min_t.setX(p.x());
			if (p.y() < min_t.y()) min_t.setY(p.y());
			if (p.z() < min_t.z()) min_t.setZ(p.z());
		}
	}

	n->endP.setCoordinates(max_t.x(), max_t.y(), max_t.z());
	n->startP.setCoordinates(min_t.x(), min_t.y(), min_t.z());
}

void Node::findDivIndex() {
	vector<double> sizes = {end().x() - start().x(), end().y() - start().y(), end().z() - start().z()};
	double maxDif = *std::max_element(sizes.begin(), sizes.end());
	divIndex = std::distance(sizes.begin(), std::find(sizes.begin(), sizes.end(), maxDif));
}

void Node::divide() {
	if (deep > MAX_DEEP) return;
	Node *l = new Node(start(), end(), this);
	Node *r = new Node(start(), end(), this);
	l->deep = this->deep + 1;
	r->deep = this->deep + 1;
	findDivIndex();

	auto tmp_r = Point(start());
	auto tmp_l = Point(end());

	Point change = (start() + end()) * 0.5;
	switch (divIndex) {
		case 0: tmp_r.setX(change.x());
			tmp_l.setX(change.x());
			break;
		case 1: tmp_r.setY(change.y());
			tmp_l.setY(change.y());
			break;
		case 2: tmp_r.setZ(change.z());
			tmp_l.setZ(change.z());
			break;
	}
	l->setEnd(tmp_l);
	r->setStart(tmp_r);
	this->setLeft(l);
	this->setRight(r);
	for (auto &t : this->trianglesList) {
		bool ans = false;
		switch (divIndex) {
			case 0: ans = t.center().x() > change.x();
				break;
			case 1: ans = t.center().y() > change.y();
				break;
			case 2: ans = t.center().z() > change.z();
				break;
		}
		if (ans) {
			r->addTriangle(t);
		} else {
			l->addTriangle(t);
		}
	}

	this->trianglesList = {};
}

void Node::findAllNodes(Ray r, vector<Node *> *n) {
	if (isRayInBox(r, this)) {
		if (left() == nullptr && right() == nullptr) {
			if (!triangles().empty()) {
				n->push_back(this);
			}
		} else {
			if (left() != nullptr) left()->findAllNodes(r, n);
			if (right() != nullptr) right()->findAllNodes(r, n);
		}
	}

}

int Node::calcNodesSize(Node *n, int c) {
	if (n->right() == nullptr && n->left() == nullptr) c += n->trianglesCount();
	else {
		c = calcNodesSize(n->left(), c);
		c = calcNodesSize(n->right(), c);
	}
	return c;
}

bool Node::isRayInBox(Ray r, Node *n) {
	double dx = 1.0 / r.direction().x();
	double dy = 1.0 / r.direction().y();
	double dz = 1.0 / r.direction().z();

	double t1 = (n->start().x() - r.origin().x()) * dx;
	double t2 = (n->end().x() - r.origin().x()) * dx;
	double t3 = (n->start().y() - r.origin().y()) * dy;
	double t4 = (n->end().y() - r.origin().y()) * dy;
	double t5 = (n->start().z() - r.origin().z()) * dz;
	double t6 = (n->end().z() - r.origin().z()) * dz;

	double tmin = std::max(std::max(std::min(t1, t2), std::min(t3, t4)), std::min(t5, t6));
	double tmax = std::min(std::min(std::max(t1, t2), std::max(t3, t4)), std::max(t5, t6));

	if (tmax < -EPSILON) return false; //box behind
	if (tmin > tmax + EPSILON) return false; //miss

	return true;
}

std::pair<Point, Point> Node::fitAllBoxes() {
	if (right() == nullptr && left() == nullptr) {
		if (trianglesCount() > 0) fitBox();
	} else {
		std::pair<Point, Point> l = left()->fitAllBoxes();
		std::pair<Point, Point> r = right()->fitAllBoxes();
		Point s = Point(std::min(std::min(l.first.x(), l.second.x()), std::min(r.first.x(), r.second.x())),
						std::min(std::min(l.first.y(), l.second.y()), std::min(r.first.y(), r.second.y())),
						std::min(std::min(l.first.z(), l.second.z()), std::min(r.first.z(), r.second.z())));
		Point e = Point(std::max(std::max(l.first.x(), l.second.x()), std::max(r.first.x(), r.second.x())),
						std::max(std::max(l.first.y(), l.second.y()), std::max(r.first.y(), r.second.y())),
						std::max(std::max(l.first.z(), l.second.z()), std::max(r.first.z(), r.second.z())));
		setStart(s);
		setEnd(e);
	}
	return {start(), end()};
}
